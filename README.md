...
传送带模型：有$W$个窗口，窗口编号为$0,1,...,W-1$，第$i(0\le i\le W-2)$个窗口到第$i+1$个窗口有一条有向边。第$t(0\le t\<W )$个窗口到第$0$个窗口有一条有向边，这条边最多只能经过$L$次，此外还存在$k$个窗口$w_0,w_1,...,w_{k-1}(t<w_0<w_1<...<w_{k-1}<W)$满足窗口$w_i$到窗口$w_i$有一条可以最多经过$L$次的有向边。
